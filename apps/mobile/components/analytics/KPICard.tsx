...
export function KPICard({
      ...
      <Card surfaceVariant="2" className="rounded-2xl border border-border/50">
        <Card.Body>
          <View className="gap-3">
            ...
            <Skeleton
              className={`${isPrimary ? "h-8" : "h-7"} w-32 rounded-md`}
            />
          </View>
        </Card.Body>
      </Card>
  ...
  return (
    <Card surfaceVariant="2" className="rounded-lg border border-border/50">
      <Card.Body>
        <View className="gap-2">
          {/* Header with title and icon */}
          <View className="flex-row items-center justify-between">
            <Text
              className="text-xs font-bold uppercase tracking-wider text-default-600 flex-1"
              numberOfLines={1}
            >
              {title}
            </Text>
          ...
          {/* Value */}
          <Text
            className={`${isPrimary ? "text-xl" : "text-xl"} text-foreground font-black tracking-tight`}
          >
            {formatValue(value)}
          </Text>
          {changeDisplay && (
