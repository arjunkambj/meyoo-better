 ...
 * Handles both ISO string dates and existing timestamps
 */
function toTimestamp(date: string | number | undefined): number | undefined {
...
/**
 * Create or update organization with proper date conversion
 */
export const upsertOrganization = mutation({
  ...
  handler: async (ctx, args) => {
    const { organizationId, ownerId, createdAt, updatedAt, ...rest } = args;
    ...
    if (existing) {
      // Update existing - convert IDs to proper types
      const { organizationId: _orgId, ...updateData } = organizationData;

      await ctx.db.patch(existing._id, {
        ...updateData,
        ownerId: updateData.ownerId
          ? (updateData.ownerId as Id<"users">)
          : existing.ownerId,
      ...
      // Create new - convert IDs and add required fields
      const { organizationId: _orgId, ...insertData } = organizationData;

      if (!insertData.ownerId) {
        throw new Error("ownerId is required to create an organization");
      }

      const orgData = {
        ...insertData,
        ownerId: insertData.ownerId as Id<"users">,
        createdAt: insertData.createdAt || Date.now(),
        updatedAt: insertData.updatedAt || Date.now(),
      };
