                ...
                {products.length === 0 ? (
                  ...
                  products.flatMap((item, idx) => {
                    ...
                    const avgPrice = Array.isArray(item.variants) && item.variants.length > 0
                      ? item.variants.reduce((sum, v) => sum + (Number(v.price ?? 0) || 0), 0) / item.variants.length
                      : item.price;
                      ...
                      >
                        <TableCell>
                          <div className="min-w-0 flex items-center gap-3 py-1">
                            ...
                            <Avatar size="sm" className="flex-none" radius="md" name={item.name} src={item.image} />
                            <div className="min-w-0">
                              <p className="font-medium text-sm truncate">{item.name}</p>
                              <p className="text-xs text-default-500 truncate">{item.vendor}</p>
                            </div>
                          </div>
                        </TableCell>
                        <TableCell><span className="text-sm">{item.category}</span></TableCell>
                        <TableCell>
                          <div>
                            <p className="text-sm font-medium">{item.available}</p>
                            {item.reserved > 0 && (
                        ...
                        <TableCell>
                          <p className="text-sm font-medium">{currencySymbol}{avgPrice.toFixed(2)}</p>
                        </TableCell>
                    ...
                    const children = item.variants.map((v) => (
                      <TableRow key={`v-${v.id}`} className={stripe ? "bg-default-50/40 pointer-events-none" : "pointer-events-none"}>
                        {/* Product column: variant title + sku */}
                        <TableCell>
                          <div className="min-w-0">
                            <div className="truncate text-sm text-default-900">{v.title || "Variant"}</div>
                            <div className="text-xs text-default-500 truncate">{v.sku || ""}</div>
                          </div>
                        </TableCell>
                        {/* Category */}
                        <TableCell><span className="text-sm text-default-500">{item.category}</span></TableCell>
                        {/* Stock */}
                        <TableCell>
                          <div>
                        ...
                        {/* Status (inherit product status) */}
                        <TableCell>{renderCell(item, "status")}</TableCell>
                        {/* Price */}
                        <TableCell>
                          <p className="text-sm font-medium">{currencySymbol}{Number(v.price || 0).toFixed(2)}</p>
                        </TableCell>
                        {/* COGS, Margin, Units, Turnover not available at variant granularity here */}
                        <TableCell>—</TableCell>
                        <TableCell>—</TableCell>
                        <TableCell>—</TableCell>
                        <TableCell>—</TableCell>
