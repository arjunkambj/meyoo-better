...
export function useCost(
  ...
  }, [dateRange, offsetMinutes, timezoneIana]);
...
export function useSaveVariantCosts() {
  ...
  return async (
    ...
    const normalizedCosts = costs.map(
      (cost): Parameters<typeof mutation>[0]["costs"][number] => ({
        variantId: cost.variantId as Id<"shopifyProductVariants">,
        cogsPerUnit: cost.cogsPerUnit,
        handlingPerUnit: cost.handlingPerUnit,
        taxPercent: cost.taxPercent,
      }),
    );

    try {
      const res = await mutation({ costs: normalizedCosts });
