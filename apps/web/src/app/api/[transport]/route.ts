    ...
    server.tool(
      ...
      "Summarize order volumes, revenue, and fulfillment performance over a date range. Returns total orders by status (pending, processing, completed, cancelled), financial metrics (revenue, costs, net profit, average order value, gross margin, CAC), fulfillment metrics (rate, avg time, returns), and period-over-period changes. Works for single-day or multi-day ranges",
      {
        apiKey: z
          .string()
          .describe(
            "API key for authentication (optional when Authorization header is present)"
          )
          .optional(),
        startDate: z
          .string()
      ...
      async ({ apiKey, startDate, endDate }, extra) => {
        try {
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.ordersSummary,
            {
              apiKey: token,
              startDate,
              endDate,
    ...
    server.tool(
      ...
      async ({ apiKey, limit }, extra) => {
        try {
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.inventoryLowStock,
            {
              apiKey: token,
              limit,
            }
          ...
          if (result.totalAlerts === 0) {
            return {
              content: [
                {
                  type: "text",
                  text: "No low-stock alerts detected. Inventory levels look healthy.",
                },
              ],
            };
          }
    ...
    server.tool(
      ...
      async ({ apiKey, startDate, endDate }, extra) => {
        try {
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.pnlSnapshot,
            {
              apiKey: token,
              startDate,
              endDate,
    ...
    server.tool(
      ...
      "Summarize store performance metrics over a date range (daily/weekly/monthly)",
      {
        ...
        metrics: z
          .array(z.string())
          .optional()
          .describe("Specific metric field names to include"),
      },
      async ({ apiKey, startDate, endDate, granularity, metrics }, extra) => {
        try {
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.analyticsSummary,
            {
              apiKey: token,
              startDate,
              endDate,
    ...
    server.tool(
      ...
      async ({ apiKey, startDate, endDate }, extra) => {
        try {
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.metaAdsOverview,
            {
              apiKey: token,
              startDate,
              endDate,
...
`;
    ...
    server.tool(
      ...
      async ({ apiKey }, extra) => {
        try {
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.getCurrentDate,
            {
              apiKey: token,
            }
          );
    ...
    server.tool(
      ...
      async ({ apiKey }, extra) => {
        try {
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.getBrandSummary,
            {
              apiKey: token,
            }
          );
    ...
    server.tool(
      ...
      async (
        { apiKey, page, pageSize, stockLevel, search, sortBy, sortOrder },
        extra
      ) => {
        try {
          const token = resolveApiToken(extra, apiKey);
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.productsInventory,
            {
              apiKey: token,
              page,
              pageSize,
    ...
    server.tool(
      ...
      async ({ apiKey, role, status, includeRemoved }, extra) => {
        try {
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.orgMembers,
            {
              apiKey: token,
              role,
              status,
    ...
    server.tool(
      ...
      "Send an email via the Resend component. Confirm the exact recipient email with the user before invoking this tool. Supports dry-run previews",
      {
        ...
        memberId: z
          ...
          .optional()
          .describe(
            "Optional organization member ID to resolve the recipient email"
          ),
        toEmail: z
          .string()
          .email()
          .optional()
          .describe(
            "Explicit recipient email (provide this when not referencing a memberId)"
          ),
        subject: z
          .string()
          .min(3)
        ...
        from: z
          ...
          .optional()
          .describe(
            "Override the default from address (defaults to Meyoo no-reply)"
          ),
        previewOnly: z
          .boolean()
          .optional()
      ...
      async (
        ...
        try {
          ...
          const result = await convexClient.action(
            api.agent.mcpActions.sendEmail,
            {
              apiKey: token,
              memberId,
              toEmail,
...
const verifyToken = async (
  ...
  try {
    const validation = await convexClient.query(
      api.web.security.validateApiKey,
      {
        key: bearerToken,
      }
    );
